...
A **Violentmonkey / Tampermonkey** userscript that transforms any web page into a **side-by-side bilingual view**: the original text on the left, the machine translation on the right. ✍️➡️🤖
...
- **Preserves page appearance** 🖼️ — Images, layout, fonts, colors, and interactive elements on the original side remain untouched.
- **Dynamic page support** ⚡ — A MutationObserver automatically translates new content added to the page (SPAs, infinite scroll, etc.).
- **Hotkey** ⌨️ — Press `Alt+Shift+T` to toggle translation on/off.
- **Language selection** 🏳️ — Built-in UI panel in the top-right corner lets you pick from 13 languages (RU, EN, UK, DE, FR, ES, IT, PT, PL, TR, ZH-CN, JA, KO).
...
**Left column** — original text ✍️, **right column** — machine translation 🤖.  
Both columns scroll **in sync** 🔄 — lines are always aligned.

---

## Installation 📥

1. Install [Violentmonkey](https://violentmonkey.github.io/) or [Tampermonkey](https://www.tampermonkey.net/) for your browser. 🛠️
...
2. Click the **↔ Translate** button in the floating UI panel (top-right corner). 🖱️
...
6. Click **Restore** to return the page to its original single-column state. ↩️
...
- **Translate page to two columns** — Start translation ▶️
- **Restore page to original** — Restore original view ↩️
...
|---|---|---|
| `targetLang` | `'ru'` | Target language code (stored in localStorage as `vmBilingualTranslator.targetLang`) 🏳️ |
